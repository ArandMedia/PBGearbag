...
export const communityService={
  ...
  async teamRoster(teamId:string){return (await apiClient.get<{userId:string;userName:string;role:string;joinedAt:string}[]>(`/teams/${teamId}/roster`)).data},
  async teamApplications(teamId:string){return (await apiClient.get<{id:string;userId:string;userName:string;message?:string;status:string;createdAt:string}[]>(`/teams/${teamId}/applications/manage`)).data},
  async decideTeamApplication(id:string,status:'approved'|'declined'){return (await apiClient.patch(`/teams/applications/${id}`,{status})).data},
