...
class MarketplaceService {
  async getListings(
    ...
    if (filters) {
      ...
      if (filters.acceptsTrades !== undefined) params.acceptsTrades = filters.acceptsTrades;
      if (filters.shippingAvailable !== undefined) params.shippingAvailable = filters.shippingAvailable;
      if (filters.localPickup !== undefined) params.localPickup = filters.localPickup;
    }
  ...
  async getFilterOptions(): Promise<{brands:{value:string;count:number}[];locations:{value:string;count:number}[];price:{min:number;max:number}}> {
    return (await apiClient.get('/marketplace/filter-options')).data;
  }
