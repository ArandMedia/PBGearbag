...
export class InitialPostgresSchema1720570000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TYPE "users_skill_level_enum" AS ENUM ('beginner','intermediate','advanced','pro')`);
    await queryRunner.query(`CREATE TYPE "listings_category_enum" AS ENUM ('marker','mask','tank','loader','apparel','accessory','complete_setup','paint','parts')`);
    await queryRunner.query(`CREATE TYPE "listings_condition_enum" AS ENUM ('new','like_new','excellent','good','fair','parts')`);
    await queryRunner.query(`CREATE TYPE "listings_status_enum" AS ENUM ('draft','active','pending','sold','removed')`);
    ...
    await queryRunner.query(`CREATE INDEX "IDX_listings_status_created" ON "listings" ("status", "created_at" DESC)`);
